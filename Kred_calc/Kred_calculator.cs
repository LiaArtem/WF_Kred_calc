...
namespace Kred_calc
{
	public partial class Kred_calculator
	{
        ...
        public List<string> curr_code_date = new List<string>();
        public List<string> curr_code_val = new List<string>();
        public static double p_sum_year;
        public static Boolean p_is_year;
        public static double p_sum_month;
        public static Boolean p_is_month;
        public static string tec_kat = Application.StartupPath;
		...
		public void Dop_plat()
		{
            ...
            double p_calc = 0;
            p_calc = p_calc + Dop_plat_in(this.comiss_bank.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.monthly_comiss_bank.Text, this.summa_ekv.Text, this.sum_kred.Text);
            this.bank_itog.Text = p_calc.ToString("#,0.00");
            //
            p_calc = 0;
            p_calc = p_calc + Dop_plat_in(this.comiss_strah1.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.comiss_strah2.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.comiss_strah3.Text, this.summa_ekv.Text, this.sum_kred.Text);
            this.strax_itog.Text = p_calc.ToString("#,0.00");
            //
            p_calc = 0;
            p_calc = p_calc + Dop_plat_in(this.comiss_notar1.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.comiss_notar2.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.comiss_notar3.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.comiss_notar4.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.comiss_notar5.Text, this.summa_ekv.Text, this.sum_kred.Text);
            this.notar_itog.Text = p_calc.ToString("#,0.00");

            //
            p_calc = 0;
            p_calc = p_calc + Dop_plat_in(this.comiss_rieltor1.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.comiss_rieltor2.Text, this.summa_ekv.Text, this.sum_kred.Text);
            p_calc = p_calc + Dop_plat_in(this.comiss_rieltor3.Text, this.summa_ekv.Text, this.sum_kred.Text);
        ...
        public double Dop_plat_in_month(string t_sum_kred)
        {
            ...
            if (tt.IndexOf("%MONTH") >= 0) { sum_out = sum_out + Dop_plat_in(tt, this.summa_ekv.Text, t_sum_kred); }
        ...
        public double Dop_plat_in_year(string t_sum_kred)
        {
            ...
            string tt;
            tt = this.comiss_strah1.Text;
            if (tt.IndexOf("%YEAR") >= 0) { sum_out = sum_out + Dop_plat_in(tt, this.summa_ekv.Text, t_sum_kred); }
            tt = this.comiss_strah2.Text;
            if (tt.IndexOf("%YEAR") >= 0) { sum_out = sum_out + Dop_plat_in(tt, this.summa_ekv.Text, t_sum_kred); }
            tt = this.comiss_strah3.Text;
            if (tt.IndexOf("%YEAR") >= 0) { sum_out = sum_out + Dop_plat_in(tt, this.summa_ekv.Text, t_sum_kred); }
        ...
        public double Dop_plat_in(string t_in, string t_summa_ekv, string t_sum_kred)
		{
			string n = t_in;
		...
		private double Prov_numeric(string text_in)
		{			
			if (double.TryParse(text_in, out double n) == false)
			{
				n = 0;
        ...
        public void Paint_table()
        {
            ...
            // Добавление колонок
            //
            DataGridViewTextBoxColumn com1 = new DataGridViewTextBoxColumn
            {
            ...
            //
            DataGridViewTextBoxColumn com2 = new DataGridViewTextBoxColumn
            {
            ...
            //
            DataGridViewTextBoxColumn com3 = new DataGridViewTextBoxColumn
            {
            ...
            //
            DataGridViewTextBoxColumn com4 = new DataGridViewTextBoxColumn
            {
            ...
            //
            DataGridViewTextBoxColumn com5 = new DataGridViewTextBoxColumn
            {
            ...
            //
            DataGridViewTextBoxColumn com6 = new DataGridViewTextBoxColumn
            {
            ...
            //
            DataGridViewTextBoxColumn com7 = new DataGridViewTextBoxColumn
            {
        ...
        public void Rashet_formula(string type_rashet, string type_annuitet)
		{
			...
			if (on_dop_rasx.Checked == true)
			{
				...
				{
					sum_kred = sum_kred + Prov_numeric(this.sum_dop_plat.Text);
				}
			}
			...
			{				
				...
				{                    
                    ...
                    if (sum_plat > Round(calc_sum_cred + pr, 2))
                    {
                        ...
                        summ = summ - (sum_plat - Round(pr, 2));
                    }
                    else
                    {
                        summ = summ - Round(calc_sum_cred, 2);
                    }

                    mass_num[4, i - 1] = sum_itog + sum_pereplata;

                    summ_graf = summ_graf - Round(calc_sum_cred, 2);
                    d_date = d_date.AddMonths(1);					 
					n_pr = n_pr + pr;
            ...
            if (this.DataGridView1.Rows.Count >= 3)
            {
                ...
                i = 0;
                typ = 0;
                etalon = this.DataGridView1.Rows[i].Cells[0].Value.ToString().Substring(0, 4);
                Get_color(i);
                for (i = 1; i <= this.DataGridView1.Rows.Count - 1 - 2; i++)
                {
                    if (this.DataGridView1.Rows[i].Cells[0].Value.ToString().Substring(0, 4) == etalon)
                    {
                        Get_color(i);
                    }
                    else
			...
			//	typ = 2;
				...
				//}
			//}
			this.DataGridView1.Rows[this.DataGridView1.Rows.Count - 1 - 1].DefaultCellStyle.BackColor = Color.LightGreen;
			this.DataGridView1.Rows[this.DataGridView1.Rows.Count - 1].DefaultCellStyle.BackColor = Color.LightBlue;
            }
        }
        ...
        private void ToCSV(DataGridView DataGridView1, string strFilePath)
        {
